...
def data_preparation():
    ...
    """
    with open(f"{gnu}spiders.bin", "rb") as g:
        spiders = pickle.load(g)
    with open(f"{gnu}clean_list.bin", "rb") as h:
    ...
    print(fin)
    with open(f"{gnu}final_data_{epoch[0]}_{epoch[1]}.bin", "wb") as i:
        pickle.dump(fin, i)
...
def db_upload():
    ...
    with open(f"{gnu}final_data_{epoch[0]}_{epoch[1]}.bin", "rb") as f:
        d = pickle.load(f)
