...
def universal_variables():
    ...
    fd = f"{gnu}final_data_{epoch[0]}_{epoch[1]}.bin"
    epoch_file = f"{gnu}gnu_ftp_data.bin"
    ini = f"{gnu}urls_{epoch[0]}_{epoch[1]}.bin"
    bins = f"{gnu}binaries/"
...
def epoch_counting_info():
    ...
    try:
        ...
        print_template(f"Couldn't move {gnu.split('/')[-1]}gnu_ftp_data.bin because of:\n[bold #F31559]{e}")
        raise SystemExit
    data_counting()
