...
def call_analysis():
    ...
    url_searcher("https://ftp.gnu.org", f"{scr}/epoch_counting/left.bin")
    shutil.copy(f"{gnu}urls_{epoch[0]}_{epoch[1]}.bin", f"{gnu}finalized.bin")
...
def names_preparation():
    ...
    """
    with open(f"{gnu}finalized.bin", "rb") as f:
        final = pickle.load(f)
    ...
    if final != []:
        ...
        with open(f"{gnu}spiders.bin", "wb") as f:
        ...
        fn = f"{gnu}final_data_{epoch[0]}_{epoch[1]}.bin"
        fn_content = []
