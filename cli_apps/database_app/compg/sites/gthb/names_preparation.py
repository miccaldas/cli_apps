...
def call_analysis():
    ...
    url_searcher("https://github.com", f"{epoch[0]}_{epoch[1]}.bin")
    shutil.copy(f"{git}urls_{epoch[0]}_{epoch[1]}.bin", f"{git}finalized.bin")
...
def names_preparation():
    ...
    """
    with open(f"{git}finalized.bin", "rb") as f:
        final = pickle.load(f)
    ...
    with open(f"{git}spiders.bin", "wb") as f:
