"""
Keyword creator for *Arch* packages. Cleans bin file data and runs KeyBERT.
In the spiders we've created commands generally expecting that we would have
a generalist 'response.css', that'll gather info from most sites, another
for 'github', and another for 'sourceforge'. We'll split 'entries in these 3
groups.'
"""
import os
import pickle
...
def generic_cleaner():
    ...
    # We create a list of name's only to simplify the
    # comprehension that'll separate the entries that
    # aren't either in 'ubuntulines' or 'srclines'.
    ub = [u[0] for u in ubs]
    sf = [s[0] for s in sfs]
    ub_sf = ub + sf
    gens = [e for e in entries if e["name"] not in ub_sf]
    # The results for these apps were nonsensical. We take them out.
    gene = [g for g in gens if g["name"] != "fh2svg" and "rst2man.py" != g["name"]]
    # Here we'll only take information from 'name' and 'content'. We create
    # a list with only that.
    genes = [[r["name"], r["content"]] for r in gene if r["content"] != []]
    # We take out linebreaks and spaces from the 'content' lists.
    gener = [[sub[0], [v.strip() for v in sub[1]]] for sub in genes]
    # The next four comprehensions remove string patterns that'll will confuse keyBert.
    ...
    # Again we'll see a code block that had manual internvention. I saw the 'content' content
    # and knew what would be easily automated and what not. In an environment where we take
    # from several and unpredictable site's, I think this will always happen.
    # In 'point' there's entries where we can get the 'presentation' value from content's
    # index[0], and some that we can't. We'll separate them in two different lists.
    # This first list is for those we can't  take the 'presentation' value in the first line.
    nofirst = [
        "vboximg-mount",
        "ppmtospu",
        "sst_dump",
    ...
    # Selection of entries that 'CAN' have the 'presentation' value from index[0]. But still not taken,
    # just segregated to their own list.
    ...
    # From 'first' we finally select name and index[0] of the list as index[1].
    genericlns = [[f[0], f[1][0]] for f in firsts]
    # We add to it the results of 'sconds'.
    genericlns += seconds
    # And take out some linebreaks that escped us.
...
def final_list():
    """
    Collects the results of the past functions
    adds them together in a new list. Adds the
    url value to each entry. This is not
    necessary in this module, but it's expected
    to be there in 'db_upload'.
    """
    ...
    # 'cleandata' has the url value and in both lists the first element
    # is the app's name.
