...
def data_preparation():
    ...
    # Cleaning up a goof I made earlier.
    for c in cleandata:
        if c[0] == "rst2latex":
            c.insert(2, "https://manpages.ubuntu.com/manpages/xenial/man1/rst2latex.1.html")

    # The next block is a sanity check. To see if everything that we got from cleandata is ok.
    print_template(cleandata)
    print("\n")
    dec = input_decision("Do you want to ontinue?{y/n}")
    if dec == "n":
        raise SystemExit
