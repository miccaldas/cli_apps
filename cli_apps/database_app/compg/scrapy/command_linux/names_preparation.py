...
def call_analysis():
    ...
    url_searcher("https://www.commandlinux.com", f"{epoch[0]}_{epoch[1]}.bin")
    shutil.copy(f"{cmd}urls_{epoch[0]}_{epoch[1]}.bin", f"{cmd}finalized.bin")
...
def names_preparation():
    ...
    """
    with open(f"{cmd}finalized.bin", "rb") as f:
        final = pickle.load(f)
    ...
    with open(f"{cmd}spiders.bin", "wb") as f:
