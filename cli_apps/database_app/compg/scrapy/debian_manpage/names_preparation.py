...
def call_analysis():
    ...
    url_searcher("https://manpages.debian", f"{scr}/epoch_counting/left.bin")
    shutil.copy(f"{deb}urls_{epoch[0]}_{epoch[1]}.bin", f"{deb}finalized.bin")
...
def names_preparation():
    ...
    """
    with open(f"{deb}finalized.bin", "rb") as f:
        final = pickle.load(f)
    ...
    with open(f"{deb}spiders.bin", "wb") as f:
