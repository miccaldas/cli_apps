...
def data_preparation():
    ...
    with open(f"{pip}clean_list.bin", "rb") as f:
        clean = pickle.load(f)
    with open(f"{pip}spiders.bin", "rb") as g:
    ...
    plnk = [f + (t[2],) for f in clean for t in spiders if f[0] == t[0]]

    kwlst = os.listdir(f"{pip}kws")
    ...
    with open(f"{pip}final_data.bin", "wb") as g:
        pickle.dump(data, g)
...
def db_upload():
    ...
    to the db.
    """
    with open(f"{pip}final_data.bin", "rb") as f:
        dt = pickle.load(f)
        name = f"{dt[0]['name']}"
