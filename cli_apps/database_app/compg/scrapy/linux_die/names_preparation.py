...
def call_analysis():
    ...
    url_searcher("https://linux.die.net", f"{epoch[0]}_{epoch[1]}.bin")
    shutil.copy(f"{die}urls_{epoch[0]}_{epoch[1]}.bin", f"{die}finalized.bin")
...
def names_preparation():
    ...
    """
    with open(f"{die}finalized.bin", "rb") as f:
        final = pickle.load(f)
    ...
    with open(f"{die}spiders.bin", "wb") as f:
